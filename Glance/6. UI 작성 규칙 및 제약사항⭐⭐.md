...
```kotlin
// ❌ 사용 불가
LazyColumn { }           // Scrollable 리스트 불가 (제한적 지원)
LazyRow { }              // Scrollable 행 불가
ConstraintLayout { }     // ConstraintLayout 불가
Scaffold { }             // Scaffold 불가
```

```kotlin
// ✅ 대안: 제한된 항목 수로 고정 레이아웃
Column {
...
// ❌ Coil/Glide 직접 사용 불가
// 대신 Worker에서 다운로드 후 URI/Bitmap으로 전달
```
...
class LoadImageWorker(context: Context, params: WorkerParameters)
    ...
    override suspend fun doWork(): Result {
        ...
        return try {
            // 1. 이미지 다운로드 (Coil/Glide 등)
            val bitmap = downloadAndResize(imageUrl, maxSize = 200)
